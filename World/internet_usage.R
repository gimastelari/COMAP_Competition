...
# Tech Investment vs Internet Usage
plot(int_usage$dataValue, ann_inv$dataValue)
...
# Combine all `victim.country.*` columns into a single `victim.country` column
vcdb_data <- vcdb_data1 %>%
    mutate(victim.country = coalesce(!!!select(., starts_with("victim.country"))))

# Keep only relevant variables
vcdb_data1 <- vcdb_data1[, c("victim.")]
